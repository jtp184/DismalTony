module DismalTony #:nodoc:
	...
	class ScheduleEvent
		...
		def ready?
			...
			!self.finished? && self =~ Time.now
		end

		# Compares a ScheduleEvent +other+ and self with minute accuracy
		def ==(other)
			comp = other.time
			comp.year == @time.year and comp.month == @time.month and comp.day == @time.day and comp.hour == @time.hour and comp.min == @time.min
		end
		...
		def =~(other)
			...
			if other.is_a? Time
				other.year == @time.year and other.month == @time.month and other.day == @time.day and other.hour == @time.hour and other.min == @time.min
			else
				nil
			end
		end
		...
		def eql?(other)
			...
			other.user_identity == self.user_identity
		end
