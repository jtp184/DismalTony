module DismalTony # :nodoc:
  ...
  class HandlerRegistry
    ...
    # Attribute reader for #handlers
    def self.handlers
      @@handlers
    end

    # Adds the QueryHandler +handler+ to #handlers
    def self.register(handler)
      @@handlers << handler
    end

    # Syntactic sugar. Calls the method on #handlers
    def self.each
      @@handlers.each
    end

    # Returns an array of QueryHandlers whose name matches +par+
    def self.[](par)
      par = Regexp.new(par, Regexp::IGNORECASE) if par.is_a? String
      @@handlers.select { |h| h.new(DismalTony::VIBase.new).handler_name =~ par }
    end

    # Returns an array of QueryHandlers whose group (if one exists) matches +par+
    def self.group(par)
